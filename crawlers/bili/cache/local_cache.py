...
class ExpiringLocalCache(AbstractCache):
    ...
    def _clear(self):
        ...
        :return:
        """
        # 先把键取成列表再删: 直接遍历 dict 边遍历边删会 RuntimeError(dictionary changed size during iteration)
        for key in list(self._cache_container.keys()):
            _, expire_time = self._cache_container[key]
            if expire_time < time.time():
                del self._cache_container[key]
